...
import { MILLISECONDS_HOUR, MILLISECONDS_MINUTE, MILLISECONDS_SECOND } from "./config"
 ...
 */
export const handleTimeAdjust = (milliseconds: number) => {
...
export const handleTimerTime = (totalMilliseconds: number, timeType: TimerTime): number => {
  let res = 0
  if (timeType === "hour") {
    res = totalMilliseconds * MILLISECONDS_HOUR
  } else if (timeType === "minute") {
    res = totalMilliseconds * MILLISECONDS_MINUTE
  } else if (timeType === "second") {
    res = totalMilliseconds * MILLISECONDS_SECOND
  } else {
